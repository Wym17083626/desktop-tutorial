...
class Ner(BertForTokenClassification):
    def __init__(self, config):
        ...
        self.classifier=conditional_forest.Conditional_forest(12,4,12,768).cuda()
...
#all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all_valid_ids,all_lmask_ids
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,valid_ids=None,attention_mask_label=None):
        ...
        logits = self.classifier(sequence_output,False)
        ...
        if labels is not None:
            ...
            if attention_mask_label is not None:
                ...
                loss = loss_fct(torch.log(active_logits), active_labels)
            else:
...
def main():
    ...
    bert_model="bert-base-cased" #"Bert pre-trained model selected in the list: bert-base-uncased, ""bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, ""bert-base-multilingual-cased, bert-base-chinese.
    task_name="ner" #The name of the task to train.
    output_dir="log/" #The output directory where the model predictions and checkpoints will be written.
    ...
    do_train=True
    do_eval=True
    eval_on="test"#"Whether to run eval on the dev set or test set.")
    ...
    classifier_oaram = list(model.classifier.named_parameters())
    no_decay = ['bias','LayerNorm.weight']
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)], 'weight_decay':  weight_decay},
        {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0},
        {'params':[p for n, p in classifier_oaram],'weight_decay': 0.0,'lr':0.01}
        ]
    ...
    if  do_eval and ( local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        report = classification_report(y_true, y_pred,digits=10)
        logger.info("\n%s", report)
